    ...
    for i, group in enumerate(teams):
        for j, topic in enumerate(topics):
            ...
            team_topic_edges.append(
                (group, topic, {"capacity": team_capacities[i], "weight": weight})
            )
        ...
        for k, topic in enumerate(topics):
            ...
            if capacity > 0 and weight > 0:
                topic_tutor_edges.append(
                    (topic, tutor, {"capacity": capacity, "weight": weight})
                )
...
def get_tutors_sink_edges(tutors, tutor_capacities, tutor_weights):
    ...
    tutor_sink_edges = [
        (
            tutor,
            SINK_NODE_ID,
            {"capacity": tutor_capacities[i], "weight": tutor_weights[i]},
        )
...
def create_edges(
    ...
    """Creates edges to create a digraph. The edges are from source node
    to teams nodes, from teams nodes to topic nodes, from topics nodes to
    tutors nodes, and from tutors nodes to sink node."""
    teams, topics, tutors = get_all_entities(num_groups, num_topics, num_tutors)
    team_topic_edges = get_teams_topics_edges(
        teams, topics, team_weights, team_capacities
    )
    source_teams_edges = get_source_teams_edges(teams)
    topic_tutor_edges = get_topics_tutors_edges(
        topics, tutors, topic_capacities, topic_weights
    )
