...
def upgrade() -> None:
    ...
    if "periods" not in table_names:
        ...
        op.create_table(
            ...
            sa.Column(
                "role", sa.Enum("STUDENT", "TUTOR", "ADMIN", name="role"), nullable=True
            ),
            sa.PrimaryKeyConstraint("id"),
        )
    ...
    op.create_index(
        op.f("ix_tutor_periods_tutor_id"),
        "tutor_periods",
        ["tutor_id"],
        unique=False,
        if_not_exists=True,
    ...
    if "groups" not in table_names:
        op.create_table(
            ...
            sa.ForeignKeyConstraint(
                ["assigned_topic_id"], ["topics.id"], ondelete="SET NULL"
            ),
            sa.ForeignKeyConstraint(
                ["tutor_period_id"],
                ["tutor_periods.id"],
    ...
    if "topics_tutor_periods" not in table_names:
        op.create_table(
            ...
            sa.ForeignKeyConstraint(
                ["tutor_period_id"], ["tutor_periods.id"], ondelete="CASCADE"
            ),
