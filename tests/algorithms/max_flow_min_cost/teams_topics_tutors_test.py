...
import time
from tests.algorithms.max_flow_min_cost.helper import create_groups, create_topics, create_tutors, create_matrix, create_vector
from src.algorithms.max_flow_min_cost import MaxFlowMinCostSolver
...
def test_more_groups_than_tutors_without_enough_capacity_so_there_are_groups_without_tutor():
    ...
    topics_tutors_weights = [
       [1, 1, 0, 0, 0, 0],  # tutors as rows
       [0, 0, 1, 1, 1, 1]   # topics as columns
    ]
    tutors_capacities = [1, 1]
    
    groups = create_groups(3, group_weights)
    topics = create_topics(6)
    tutors = create_tutors(2, tutors_capacities, topics_tutors_capacities, topics_tutors_weights)
    solver = MaxFlowMinCostSolver(groups, topics, tutors)
...
def test_equal_groups_and_tutors_but_tutors_do_not_exceed_their_capacities():
    ...
    groups = create_groups(3, group_weights)
    topics = create_topics(6)
    tutors = create_tutors(3, tutors_capacities, topics_tutors_capacities, topics_tutors_weights)
    solver = MaxFlowMinCostSolver(groups, topics, tutors)
    _groups, _topics, tutors = solver.solve()
    assert len(tutors["p1"]) <= 1
    assert len(tutors["p2"]) <= 1
...
def test_more_tutors_than_groups_but_tutors_do_not_exceed_their_capacities():
    ...
    groups = create_groups(2, group_weights)
    topics = create_topics(6)
    tutors = create_tutors(3, tutors_capacities, topics_tutors_capacities, topics_tutors_weights)
    solver = MaxFlowMinCostSolver(groups, topics, tutors)
    _groups, _topics, tutors = solver.solve()
...
def test_more_groups_than_topics_and_tutors_but_tutor_with_enough_capacity_so_every_team_is_assigned_to_one_topic():
    ...
    groups = create_groups(2, group_weights)
    topics = create_topics(1)
    tutors = create_tutors(1, tutors_capacities, topics_tutors_capacities, topics_tutors_weights)
    solver = MaxFlowMinCostSolver(groups, topics, tutors)
    groups, _topics, _tutors = solver.solve()
    assert len(groups.items()) == 2
...
def test_groups_with_same_preferences_and_weights_and_tutors_with_capacity_are_assigned_to_the_same_topic():
    """Testing groups with same preferences and weights are assigned to the same topic since 
...
def test_groups_with_same_preferences_but_tutor_capacity_for_topic_is_not_enough_so_are_not_assigned_to_the_same_topic():
    """Testing groups with same preferences and weights are not assigned to the same topic which 
...
def test_four_groups_and_topics():
    ...
    topics_tutors_weights = create_matrix(num_tutors, num_topics, False, 1)

    start_time = time.time()
    groups = create_groups(num_groups, group_weights)
    topics = create_topics(num_topics)
    tutors = create_tutors(num_tutors, tutors_capacities, topics_tutors_capacities, topics_tutors_weights)
    solver = MaxFlowMinCostSolver(groups, topics, tutors)
    groups, _topics, _tutors = solver.solve()
    end_time = time.time()
    assert len(groups.items()) > 0
    print("4 groups, 4 topics, 2 tutors - Execution time:", end_time - start_time, "seconds")
...
def test_ten_groups_and_topics():
    ...
    end_time = time.time()
    assert len(groups.items()) > 0
    print("10 groups, 10 topics, 5 tutors - Execution time:", end_time - start_time, "seconds")
...
def test_twenty_groups_and_topics():
    ...
    end_time = time.time()
    assert len(groups.items()) > 0
    print("20 groups, 20 topics, 10 tutors - Execution time:", end_time - start_time, "seconds")
...
def test_forty_groups_and_topics():
    ...
    end_time = time.time()
    assert len(groups.items()) > 0
    print("40 groups, 40 topics, 20 tutors - Execution time:", end_time - start_time, "seconds")
...
def test_eighty_groups_and_topics():
    ...
    end_time = time.time()
    assert len(groups.items()) > 0
    print("80 groups, 80 topics, 40 tutors - Execution time:", end_time - start_time, "seconds")
...
def test_one_hundred_and_sixty_groups_and_topics():
    ...
    end_time = time.time()
    assert len(groups.items()) > 0
    print("160 groups, 160 topics, 80 tutors - Execution time:", end_time - start_time, "seconds")
...
def test_three_hundred_and_twenty_groups_and_topics():
    ...
    end_time = time.time()
    assert len(groups.items()) > 0
    print("320 groups, 320 topics, 160 tutors - Execution time:", end_time - start_time, "seconds")
