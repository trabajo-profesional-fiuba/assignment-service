...
import time
from tests.algorithms.flow_solver.helper import create_groups, create_topics, create_tutors, create_matrix, create_vector
from src.algorithms.topic_tutor_assignment_flow_solver import TopicTutorAssignmentFlowSolver
...
def test_more_groups_than_tutors_without_enough_capacity_so_there_are_groups_without_tutor():
    ...
    tutors = create_tutors(2, tutors_capacities, topics_tutors_capacities, topics_tutors_costs)
...
def test_equal_groups_and_tutors_but_tutors_do_not_exceed_their_capacities():
    ...
    groups = create_groups(3, group_costs)
    topics = create_topics(6)
    tutors = create_tutors(3, tutors_capacities, topics_tutors_capacities, topics_tutors_costs)
    solver = TopicTutorAssignmentFlowSolver(groups, topics, tutors)
    _groups, _topics, tutors = solver.solve()
    assert len(tutors["p1"]) <= 1
    assert len(tutors["p2"]) <= 1
...
def test_more_groups_than_topics_and_tutors_but_tutor_with_enough_capacity_so_every_team_is_assigned_to_one_topic():
    ...
    groups = create_groups(2, group_costs)
    topics = create_topics(1)
    tutors = create_tutors(1, tutors_capacities, topics_tutors_capacities, topics_tutors_costs)
    solver = TopicTutorAssignmentFlowSolver(groups, topics, tutors)
    groups, _topics, _tutors = solver.solve()
    assert len(groups.items()) == 2
...
def test_four_groups_and_topics():
    ...
    tutors = create_tutors(num_tutors, tutors_capacities, topics_tutors_capacities, topics_tutors_costs)
    solver = TopicTutorAssignmentFlowSolver(groups, topics, tutors)
    groups, _topics, _tutors = solver.solve()
    end_time = time.time()
